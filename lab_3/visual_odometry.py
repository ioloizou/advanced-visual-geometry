...
class VisualOdometry:
    ...
    def compute_relative_transform(self, matches: List[cv2.DMatch], update_src_frame: bool) -> \
        ...
        # compute homography
        mat_H, mask = cv2.findHomography(src_pts, incoming_pts, method = cv2.RANSAC, ransacReprojThreshold = self.homography_ransac_threshold)
        ...
        src_pts_normalized = (self.inv_K @ src_pts_homo.T).T  # <num_inliers, 3>
        ...
        if num_candidates > 1:
            ...
            if len(rots) > 1:
                ...
                angle_0 = np.arccos(np.dot(normals[0].T, self.plane_normal_src) / (np.linalg.norm(normals[0]) * np.linalg.norm(self.plane_normal_src)))  # TODO: compute angle between 1st candidate of normal vector with self.plane_normal_src
                angle_1 = np.arccos(np.dot(normals[1].T, self.plane_normal_src) / (np.linalg.norm(normals[1]) * np.linalg.norm(self.plane_normal_src)))  # TODO: compute angle between 2nd candidate of normal vector with self.plane_normal_src
        ...
        if rots:
            ...
            if self.plane_d_src < 0:
                ...
                self.plane_d_src = np.dot(self.plane_origin_src.T, self.plane_normal_src).item()  # TODO: compute distance from the 1st camera frame C0 to the plane
            ...
            if update_src_frame:
                ...
                src_pts_3d = src_pts_normalized * depth[:, np.newaxis]  # TODO: compute 3d coordinate in src camera frame for every inlier,
